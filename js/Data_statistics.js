...
// var huodongurl = "http://192.168.1.15:8083/youbaoRoom3/"; //摇奖的 = 数据统计 线下
var huodongurl = "http://39.97.176.41:8081/youbaoRoom3/"; //摇奖的 = 数据统计 线上
// const $WebSocketURL = "192.168.1.15:20819/"; //摇奖的 websocket
...
cola(function(model) {
	...
	function Lottery_Just_entered() {
		...
		$.ajax({
			type: "post",
			url: huodongurl + "statistics/findToto",
			data: {
				userName: "",
				roomid: 3
			},
			...
			success: function(response) {
				log(response);
				if(response.code == "0"){
					let mydata = response.msg;
					for(var i=0;i<mydata.length;i++){
						if(mydata[i].type == "share"){
							$("#fenxiang").text(mydata[i].counts);
						}else if(mydata[i].type == "line"){
							$("#zaixian").text(mydata[i].counts);	
						}else if(mydata[i].type == "fwl"){
							$("#shiji").text(mydata[i].counts);
						}
					}
				}
	...
	function Just_entered() {
		...
		$.ajax({
			type: "post",
			url: huodongurl + "initConfig/findInitConfig",
			// url:"http://192.168.1.181:8080/YbSys/User/select",
			dataType: 'json',
			success: function(response) {
				log(response);
				$("#chushi").val(response.msg.initCount);
				gangurId = response.msg.id;
				$("#xunimin").val(response.msg.satatNum);
				$("#xunimax").val(response.msg.endNum);
				console.log("初始访问量"+$("#chushi").val());
				console.log("初始访问量min"+$("#xunimin").val());
				console.log("初始访问量max"+$("#xunimax").val());
			}
		})
	};
	// 保存的
	$("#Mysubmitlive_streamNew").click(()=>{
		...
		console.log("初始访问量min"+$("#xunimin").val());
		console.log("初始访问量max"+$("#xunimax").val());
		// http://192.168.1.15:8083/youbaoRoom3/initConfig/updateInitConfig
			$.ajax({
			type: "post",
			url: huodongurl + "initConfig/updateInitConfig",
			data:{
				id:gangurId,
				 initCount:$("#chushi").val(),
				 satatNum:$("#xunimin").val(),
				  endNum:$("#xunimax").val(),
			},
			// url:"http://192.168.1.181:8080/YbSys/User/select",
			dataType: 'json',
