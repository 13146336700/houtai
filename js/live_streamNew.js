...
cola(function(model) {
	...
	// 	// $(this)[0].elem[0].value = `${value} ${geTtimres()}`;
	// 	// if($("#test1").val() != ""){
	// 	// }
	// 		// $("#test1").val(`${$("#test1").val()} ${geTtimres()}`)
	//  },
	// });
	...
	// 		$(this)[0].elem[0].value = "2033-03-13";
	// 	},
	// 	type: 'datetime'
	// });
	...
	$("#Mysubmitlive_streamNew").click(() => {
		if(!$("#biaoti").val()){
			// alert("直播标题不得为空");
			xcsoft.info('直播标题不得为空',2000);
			return;
		}
		if(!$("#hudongzhanghao").val()){
			xcsoft.info('邮宝互动账号不得为空',2000);
			return;
		}
		if(!$("#test1").val()){
			xcsoft.info('开播时间不得为空',2000);
			return;
		}
		if(!$("#test2").val()){
			xcsoft.info('结束时间不得为空',2000);
			return;
		}
		
		
		$.ajax({
			url: url + "exercises/save",
			type: "post",
			data: {
				title: $("#biaoti").val(),
				hdId: $("#myselect option:selected").text(),
			...
			success: function(data) {
				...
				if (data.code == "ok") {
					...
					setTimeout(()=>{
						window.location = "live_stream.html";
					},1000)
				}
			},
			// cb:function(data){
	...
	$.ajax({
		...
		success: function(data) {
			...
			for (var i = 0; i < data.length; i++) {
				$("#myselect").append("<option value=" + data[i].value + ">" + data[i].key + "</option>");
				form.render('select');
			}
